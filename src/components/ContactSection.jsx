...
export const ContactSection = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const subject = encodeURIComponent(`Portfolio Dispatch from ${formData.name}`);
    const body = encodeURIComponent(
      `Name: ${formData.name}\nEmail: ${formData.email}\n\nMessage Payload:\n${formData.message}`
    );
    window.open(`mailto:infantashil55@gmail.com?subject=${subject}&body=${body}`, '_blank');
    setSent(true);
  };
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
            ...
            {sent ? (
              <div className="py-16 text-center space-y-4">
                ...
                <button
                  ...
                  className="mt-4 text-[11px] font-mono tracking-widest text-[#D4AF37] hover:underline uppercase"
                >
        ...
        {/* Centered Direct Links (Matching Reference) */}
        <div className="pt-20 mt-16 border-t border-[#8C6D4F]/20 flex items-center justify-center gap-10 sm:gap-16 md:gap-24">
          <a
            href="mailto:infantashil55@gmail.com"
            title="infantashil55@gmail.com"
            className="group text-xs sm:text-[13px] md:text-sm tracking-[0.35em] uppercase text-[#A8988B] hover:text-[#F7E7C4] transition-colors flex items-center gap-2 cursor-pointer"
            style={{ fontFamily: "'Montserrat', sans-serif" }}
          >
            <span>EMAIL</span>
            <span className="text-[#D4AF37] text-sm transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">
              ↗
            </span>
          </a>
        ...
        <div className="pt-10 mt-10 border-t border-[#8C6D4F]/10 flex flex-col sm:flex-row items-center justify-between text-center sm:text-left gap-4">
